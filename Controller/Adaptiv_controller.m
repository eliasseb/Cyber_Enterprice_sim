function [ tau,phi,omega_n,z_1,tau_L,tau_U,Chi_tilde ] = Adaptiv_controller(Eta,nu,nu_dot,phi,omega_n,Eta_d,Eta_ddot,Eta_ddotdot,tau,h)
...
z_1=R'*(Eta_tilde);
z_1_dot=Rot_dot(nu)'*Eta_tilde + R'*Eta_tilde_dot;

Alpha_1=(-K_p*z_1+(R')*Eta_ddot);
Alpha_1dot=-K_p*z_1_dot+Rot_dot(nu)'*Eta_ddot + R'*Eta_ddotdot; %%%%%%%%%%% Tatt bort 

z_2=nu-Alpha_1;

%% estimering av Phi
phi_dot=@(phi_v,v)(-Gamma_phi*Phi(v)'*(z_2)*0);
...
%%
tau_L=-z_1-K_d*z_2;
tau_U=C_c(nu)*nu+G_d(nu)+Phi(nu)*phi+M_c*Alpha_1dot-R'*omega_n;
